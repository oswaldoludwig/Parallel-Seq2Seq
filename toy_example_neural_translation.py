...
from Par_seq2seq import initialize
from Par_seq2seq import operations                                       
...
with tf.Session() as session:
        ...
        for i in range(Batch_size):  #  loop over batches of training data
            ...
            x = X[i:i+1,:,:]
            y = Y[i:i+1,:,:]
            decoded = op.greedy_decoder(x, BOS_vector, EOS_vector)

            print('input: ' + str(indx2char(onehot2indx(x[0, :, :]))))
            print('target: ' + str(indx2char(onehot2indx(y[0, :, :]))))
            print('decoded: ' + str(indx2char(onehot2indx(decoded[0]))))
